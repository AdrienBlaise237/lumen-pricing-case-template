...
export function renderPositioningModule(container, data) {
  ...
  container.innerHTML = `<section class="positioning-module" aria-labelledby="positioning-title">
    <header class="positioning-header"><p class="positioning-kicker">Independent module / Exhibits 2, 4, 5 & 10</p><h2 id="positioning-title">Win where clean energy<br /><em>can credibly perform.</em></h2><p><strong>Positioning hypothesis to test:</strong> ${output.recommendedPositioning}</p></header>
    <section class="positioning-tradeoff"><div><p class="positioning-kicker">Explicit trade-off to validate</p><h3>Accessible premium,<br />not boutique wellness.</h3></div><p>${output.strategicTradeOff}</p></section>
    ...
    <section class="positioning-grid"><article class="positioning-card"><p class="positioning-kicker">Competitive price context</p><table><thead><tr><th>Brand</th><th>Position</th><th>Price band</th></tr></thead><tbody>${output.competitiveContext.map((row) => `<tr><td>${row.competitor}</td><td>${row.positioning}</td><td>${euro.format(row.lowPriceEur)}–${euro.format(row.highPriceEur)}</td></tr>`).join('')}</tbody></table><p class="positioning-note"><strong>Positioning hypothesis:</strong> test a performance-relevant, clean-label premium. The price bands show the reference market; they do not establish LUMEN’s final price or position.</p></article><article class="positioning-card"><p class="positioning-kicker">Qual + quant reconciliation</p>${output.qualitativeReconciliation.map((item) => `<section class="positioning-reconciliation"><h3>${item.segment}</h3><p>${item.conclusion}</p><ul>${item.quotes.map((quote) => `<li>“${quote}”</li>`).join('')}</ul></section>`).join('')}</article></section>
